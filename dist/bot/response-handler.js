...
var StreamFile = require(path.join(root, 'bot/stream-file'));
...
externals.generateAlertResponseTemplate = function (context) {
  return context.template ? context.template(context) : alertTemplate;
};

externals.generateBotResponseTemplate = function (context) {
  return context.template ? context.template(context) : helpTemplate;
};
...
internals.getContextualErrorResponse = function (botCommand, context) {
  ...
  return _.reduce(_.cloneDeep(_.filter(botCommand, function (n, key) {
