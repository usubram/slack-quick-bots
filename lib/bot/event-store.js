...
externals.EventStore = class {
  ...
  */
  update (eventType, data) {
    return storage.updateEvents(this.botName, eventType, data)
      .then((eventData) => {
        this.eventStore[eventType] = eventData[this.botName];
  ...
  * @return {object} Event data.
  */
  remove (eventType, data) {
    return storage.removeEvents(this.botName, eventType, data)
      .then((eventData) => {
        this.eventStore[eventType] = eventData[this.botName];
        return eventData[this.botName];
      });
...
module.exports = externals.EventStore;
