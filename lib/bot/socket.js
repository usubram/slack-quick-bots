...
var Socket = class {
    ...
    return Promise.resolve({
      then: ((onFulfill, onReject) => {
        try {
          ...
          botLogger.logger.error('Error establishing socket connection due to ', err);
        }
  ...
  registerSocketEvents () {
    return Promise.all([
      ...
      botLogger.logger.error('Registering socket events failed due to ', err);
    });
  }
      ...
      then: ((onFulfill) => {
        this.ws.on('message', (data) => {
          ...
          try {
            ...
            botLogger.logger.error('Invalid slack data ', data);
          }
        ...
        this.ws.on('error', (err) => {
          botLogger.logger.error('Socket connection errored-out due to ', err);
        });
    ...
    return Promise.resolve({
      then: ((onFulfill) => {
        this.ws.on('open', () => {
          ...
          botLogger.logger.info('Socket connection successfully made for ', this.botName);
  ...
  clearPingPongTimer () {
    if (this.wsPingPongTimmer) {
      ...
      botLogger.logger.debug('Socket connection closed during ping for ', _.get(this, 'botName'));
  ...
  sendMessage (message, callback) {
    try {
      ...
      botLogger.logger.error('Error sending message to slack ', err);
    }
  }
};
