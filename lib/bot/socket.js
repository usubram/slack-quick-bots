...
const { extend, get, nth, compact, isEmpty } = lodash;
...
const Socket = class {
  ...
  connectSocket() {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        try {
          if (this.agent) {
            this.ws = new WebSocket(this.slackData.url, {
              agent: this.agent,
            });
          } else {
            this.ws = new WebSocket(this.slackData.url);
  ...
  setupSocketConnectionEvents(slackData) {
    extend(this.slackData, slackData);
    this.botName = get(this.slackData, 'self.name');
    this.id = get(this.slackData, 'self.id');
    this.eId = get(this.slackData, 'self.enterprise_user.id');
      ...
      then: (onFulfill) => {
        this.ws.on('message', (data) => {
          ...
          try {
            ...
            logger.debug('Slack socket message events', slackMessage);
          } catch (err) {
  ...
  registerOnCloseEvent() {
    return Promise.resolve({
      then: (onFulfill) => {
        this.ws.on('close', () => {
          logger.debug('Bot connection closed for ', this.botName);
          if (this.isShutdown || this.id === 'U1234567') {
            this.isShutdown = false;
  ...
  routeMessageType(message) {
    ...
    const messageType = nth(compact(regexResponse), 1);

    switch (messageType) {
      case 'message': {
        /* jshint ignore:start */
        const isBotMessage =
          isEmpty(message.reply_to) &&
          !message.subtype &&
