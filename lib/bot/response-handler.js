...
externals.generateErrorTemplate = function (botName, botCommand, context) {
  ...
  try {
    ...
    botLogger.logger.info('Invalid handler template', err);
  }
...
externals.processFile = function (context, botToken) {
  ...
  context.message.config).then((response) => {
    if (response.ok !== true) {
      botLogger.logger.error('Error responseType ', response);
    }
  }).catch((err) => {
    botLogger.logger.error('Error uploading file to slack ', err);
  });
};
