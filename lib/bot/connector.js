...
const { getUserInfo } = require('../slack-api/users');
...
const Connector = class {
  ...
  */
  connect () {
    ...
    postData = querystring.stringify(_.assign(CONFIG.postData, {
      token: this.token,
    }));
    ...
    return this.makeRequest(options, postData).then((slackData) => {
      ...
      return getUserInfo(userOptions).then((userInfo) => {
        return Object.assign(slackData, userInfo.user);
      });
    }).then((slackData) => {
  ...
  */
  makeRequest (options, postData) {
    return handleRequest(options, postData);
  }
