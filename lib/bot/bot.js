...
const alertTemplateSrc = fs.readFileSync(alertFilePath, 'utf8');
  ...
  if (this._isDirectMessage(message) ||
    ...
    this.command.validateCommand(parsedMessage, (validationResult, errorContext) => {
      if (validationResult) {
        ...
        this.command.eventEmitter.on('trigger:alert', (context) => {
          console.log('trigger:alert');
          this._respondForAlertCommand(parsedMessage, context);
        });
...
internals.Bot.prototype._respondForAlertCommand = function (parsedMessage, data) {
  this._dispatchMessage(this.ws,
    parsedMessage.channel,
    this._generateAlertResponseTemplate(parsedMessage, data));
};
...
internals.Bot.prototype._generateAlertResponseTemplate = function (parsedMessage, context) {
  var template = handlebars.compile(alertTemplateSrc);
  var data = {};
