...
const { CommandFactory } = require('../command/command-factory');
const EventEmitter = require('events').EventEmitter;
const { EventStore } = require('./event-store');
const { Hook } = require('./hook');
const HttpsProxyAgent = require('https-proxy-agent');
const { MockConnector } = require('./mock-connector');
const messageParser = require('../command/message');
const storage = require('../storage/storage');
const responseHandler = require('./response-handler');
const channelsApi = require('../slack-api/channels');
const usersApi = require('../slack-api/users');
const userGroupsApi = require('../slack-api/user-groups');
...
const Bot = class {
  ...
  constructor(bot) {
    ...
    this.botMessageParser = messageParser.parse(
      _.map(_.get(this.config, 'botCommand'), (command, key) => {
        return {
  ...
  handleMessage(message) {
    ...
    const isMessageForBot =
      ...
      _.toUpper(this.getEId()) === parsedMessage.message.commandPrefix;
    const isSelf =
      parsedMessage.user === _.toUpper(this.getId()) ||
      parsedMessage.user === _.toUpper(this.getEId());
    ...
    if (
      !isSelf &&
      (responseHandler.isDirectMessage(message) || isMessageForBot)
    ) {
      logger.info('processed message ', parsedMessage);
  ...
  handleErrorMessage(botName, context) {
    const renderedData = responseHandler.generateErrorTemplate(
      botName,
  ...
  getSocketAgent() {
    if (!this.socketAgent && this.proxy && this.proxy.url) {
      const opts = url.parse(this.proxy.url);
  ...
  registerHandlebarsHelpers() {
    return {
      ...
      joinArray: (context, options) => {
        ...
        return (
          prefix + (_.map(arrayVal, _.toLower) || []).join(separator) + suffix
        );
      },
      arrayItem: (context) => {
      ...
      toPlural: (context) => {
        ...
        const isMatched = _.compact(
          _.map(['s', 'x', 'z', 'ch'], (lastChr) => {
  ...
  getSlackUserProfile(identifiers, options = {}) {
    ...
    const emailIdLocalPart = _.nth(_.split(_.toUpper(email), '@'));
    const user = _.find(this.getSlackData().members, (member) => {
      const profileEmail = _.toUpper(_.get(member, 'profile.email'));
      return (
        (profileEmail === email ||
          ...
          (options.matchLocalPart &&
            emailIdLocalPart === _.nth(_.split(profileEmail, '@')))) &&
        !_.get(member, 'deleted')
      );
    });
  ...
  getSlackIdFromEmailLocalPart(emailId) {
    const uEmailId = _.nth(_.split(_.toUpper(emailId), '@'));
    const user = _.find(this.getSlackData().members, (member) => {
      return (
        _.nth(_.split(_.toUpper(_.get(member, 'profile.email', '')), '@')) ===
        uEmailId
      );
    });
