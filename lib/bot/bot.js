...
externals.Bot = class {
  ...
  */
  async handleMessage (message) {
    ...
    try {
      slackUser = await this.fetchUserInfo(options);
    } catch(err) {
      logger.error(err);
    }

    if(this.config.allowedADGroupsOnly) {
      const ad = new ActiveDirectory(this.config.activeDirectory);

      _.forEach(this.config.allowedADGroupsOnly, (groupName)=> {
        ad.isUserMemberOf(slackUser.user.name, groupName, (err, isMember)=> {
          if(err) {
    ...
    if((!isAllowedUser && this.config.allowedADGroupsOnly) && !isSelf &&
        (responseHandler.isDirectMessage(message) || isMessageForBot)) {
      return this.handleErrorMessage(this.getBotName(),
        {parsedMessage,
          notInGroup: true,
          groups: this.config.allowedADGroupsOnly});
    }
    ...
    if (!isSelf &&
        (responseHandler.isDirectMessage(message) || isMessageForBot)) {
      logger.info('processed message ', parsedMessage);
      return this.commandFactory.handleMessage(parsedMessage)
        .catch((err) => {
  ...
  */
  ...
  /**
   * Function to get user info from the slack api.
   * @param {object} config Config for get users info .
   *
   * @return {object} object containing slack data for a user.
   * Look at slack docs for users model
   */
  fetchUserInfo (config) {
    if(_.get(config, 'info', false)) {
      logger.info('Excluding call for info of user');
      return{};
    }
    logger.info('Fetching user info from slack');

    return usersApi.getUserInfo(config);
  }
