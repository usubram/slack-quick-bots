...
externals.Bot = class {
  ...
  */
  fetchTeamUsers (config) {
    ...
    return usersApi.getUsersList(_.extend({
  ...
  */
  fetchTeamChannels (config) {
    ...
    return channelsApi.getChannelsList(_.extend({
      botToken: this.config.botToken,
    }, config));
  }
  ...
  */
  getAllTeamData (config) {
    return Promise.all([
      this.fetchTeamChannels(_.get(config, 'channel')),
      this.fetchTeamUsers(_.get(config, 'user')),
    ]);
  }
