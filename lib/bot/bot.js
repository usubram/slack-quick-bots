...
externals.Bot = class {
  ...
  */
  handleMessage (message) {
    ...
    if (this.config.blockDirectMessage &&
      ...
      return this.handleBotMessages(this.config.blockDirectMessage,
        parsedMessage);
    }

    if (responseHandler.isDirectMessage(message) || isMessageForBot) {
      logger.info('processed message ', parsedMessage);
  ...
  * @param {string} message Custom message for block direct message.
  * @param {object} parsedMessage Message returned @link command/message.js.
  *
  * @return {object} Promise resolves the rendered error message.
  */
  handleBotMessages (message, parsedMessage) {
    let renderedMessage;
    try {
      renderedMessage = message ? handlebars.compile(message)({
  ...
  registerHandlebarsHelpers () {
    return {
      ...
      formatUnixTime: (context, options) => {
        let time = _.get(context, 'hash.value', 0);
        let offset = Number(_.get(context, 'hash.offset', 0));
        let format = _.get(context, 'hash.format', 'YYYY-MM-DD HH:mm');

        return moment.unix(time).utcOffset(offset).format(format);
      },
      joinArray: (context, options) => {
        let seperator = _.get(options, 'hash.seperator', ' ');
        let arrayVal = context || [];

        return (_.map(arrayVal, _.toLower) || []).join(seperator);
      },
      arrayItem: (context, options) => {
        let index = _.get(context, 'hash.index', 0);
        let arrayVal = _.get(context, 'hash.value', 0);
