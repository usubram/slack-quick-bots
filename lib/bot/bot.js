...
externals.Bot = class {
  ...
  init () {
    if (_.get(this.config, 'mock')) {
      botLogger.logger.debug('Connecting for bot in mock');
      ...
      botLogger.logger.debug('Setting up connection for bots');
    ...
    this.connectionManager.connect().catch((err) => {
      botLogger.logger.error('Failed to establish connection %j', err);
    });
  ...
  handleMessage (message) {
    ...
    if (this.config.blockDirectMessage && !responseHandler.isPublicMessage(message)) {
      botLogger.logger.info('processed message: ', parsedMessage);
  ...
  loadSavedEvents () {
    if (this.eventStore) {
      ...
      storage.getEvents(['events', 'schedule']).then((events) => {
        ...
        botLogger.logger.error('Error loading saved event %j', err);
        this.commandFactory.loadCommands();
        this.botInterface.emit('connect');
      });
  ...
  dispatchMessage (options, callback) {
    ...
    _.forEach(options.channels, (channel) => {
      ...
      try {
        ...
        botLogger.logger.error('Error sending message ', err);
      }
  ...
  start () {
    this.connectionManager.connect().catch((err) => {
      botLogger.logger.error('Unable to start the bot %j', err);
    });
  }
  ...
  injectMessage (messageObj) {
    ...
    return this.handleMessage(message).catch((err) => {
      botLogger.logger.error('Unable to inject message %j', err);
    });
  }
