  ...
  registerHandlebarsHelpers () {
    return {
      ...
      joinArray: (context, options) => {
        let separator = _.get(options, 'hash.separator', ' ');
        let arrayVal = context || [];

        return (_.map(arrayVal, _.toLower) || []).join(separator);
      },
      ...
      toPlural: (context) => {
        let length = _.get(context, 'hash.len', 0);
        let word = _.get(context, 'hash.word');
