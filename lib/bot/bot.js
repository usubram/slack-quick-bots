...
internals.Bot.prototype._handleMessage = function (message) {
  ...
  if (this.responseHandler._isDirectMessage(message) ||
    ...
    this.command.validateCommand(parsedMessage, (err) => {
      if (err) {
        this.handleErrorMessage(this.botName, err, parsedMessage);
      } else {
...
internals.Bot.prototype._dispatchMessage = function (channels, message, type, callback) {
  var messageCallback = _.isFunction(callback) ? callback : undefined;
...
internals.Bot.prototype._registerEvents = function () {
  ...
  this.command.eventEmitter.on('command:data:graph', (context) => {
    this.responseHandler.generateGraph(context, this.config.botToken);
  });
  ...
  this.command.eventEmitter.on('command:hook:respond', (context) => {
    this._dispatchMessage(context.channels, context.data.hook);
  });
