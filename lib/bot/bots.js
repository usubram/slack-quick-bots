        ...
        function (commandAttr, commandAttrkey) {
          ...
          if (commandAttrkey === 'commandType') {
            ...
            if (_.includes(['ALERT', 'RECURSIVE'], command)) {
              if (command === 'ALERT') {
                if (_.get(value, 'allowedParam', []).length > 0) {
                  value.allowedParam = _.concat([internals.alertParams],
                    value.allowedParam);
                } else {
          ...
          } else if (commandAttrkey === 'paramsHelpMessage' &&
            _.isArray(value[commandAttrkey])) {
            value[commandAttrkey] = _.map(value[commandAttrkey],
              (validation) => {
                if (validation.error) {
                  validation.error = handlebars.compile(validation.error);
                }
