...
externals.Commands = class {
  ...
  */
  setDefaultParams (defaultValues, slackResponse) {
    const param = _.compact(internals.getParams(slackResponse, null, true));
  ...
  validateCommandArgs (command, parsedMessage) {
        ...
        (rule) => {
          ...
          return _.merge({}, rule, {
            help: _.map(rule.schema, (schema, index) => {
              ...
              validationResultValue += help.result.failed ? position : 0;
              noOfErrors += Number(help.result.failed);
              return help;
            }, []),
          }, {validationResultValue, noOfErrors});
        }), (order) => - order.validationResultValue + order.noOfErrors));

    const validResult = _.filter(validationResult,
      {validationResultValue: 0});
    ...
    if (!result.isValid) {
      ...
      parsedMessage.message.params = this.setDefaultParams(
        _.nth(_.filter(validationResult,
          {validationResultValue: 0})).default, parsedMessage);
    }
...
internals.getParams = function (slackResponse, level, realValue = false) {
