...
externals.Command.prototype.validateCommand = function (slackResponse, cb) {
  ...
  } else if (!this._isCommandAllowed(slackResponse)) {
    cb(false, {restricted_user: true, users: commandModel.allowedUsers, parsedMessage: slackResponse });
  } else {
    cb(true);
  }
};
...
externals.Command.prototype.getData = function (slackResponse) {
  ...
  try {
    ...
    botLogger.logger.error('Command: error calling handler, make sure to pass a proper function', err);
  }
};
...
externals.Command.prototype.getDataForAlert = function (slackResponse) {
  ...
  try {
    ...
    botLogger.logger.error('Command: error calling handler, make sure to pass a proper function');
  }
};
...
externals.Command.prototype._isCommandAllowed = function (slackResponse) {
  var commandModel = this.commands[slackResponse.message.command];
  if (commandModel && commandModel.allowedUsers) {
    var currentUser = _.find(this.slackData.users, { 'id': slackResponse.user });
    if (currentUser) {
