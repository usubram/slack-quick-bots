...
const Command = class {
  ...
  validate(slackResponse) {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        ...
        const blockDirectMessage = this.shouldBlockDirectMessage(this.getCommand(), slackResponse);
   ...
   */
  getTemplate() {
    let template = this.getBotConfig().botCommand[this.getCommandName()]
      .template;
  ...
  getScheduledEvents(parsedMessage) {
    ...
    const allowedEvents = _.filter(_.values(events), (item) => {
      ...
      const isSchedule = !!_.get(item, 'parsedMessage.scheduleId');
      const isSameUser =
        _.get(item, 'parsedMessage.user') === _.get(parsedMessage, 'user');
      const isPublicSchedule = isSchedule
        ? responseHandler.isPublicMessage(item.parsedMessage)
        : false;
    ...
    return _.map(allowedEvents, (item) => {
      const context = {
        ...
        private: !responseHandler.isPublicMessage(_.get(item, 'parsedMessage')),
  ...
  validateCommandArgs(command, parsedMessage) {
    ...
    const validationResult = _.compact(
      _.orderBy(
        _.map(commandValidations, (rule) => {
    ...
    const targetError =
      command.commandType === 'FLOW'
        ? _.last(validationResult)
        : _.first(validationResult);

    if (!result.isValid) {
      _.merge(result, {
        noOfErrors: _.get(targetError, 'noOfErrors'),
        cause: _.pick(targetError, ['schema', 'help']),
        failedParams: _.compact(
          _.map(targetError.help, (item) => {
            if (item.result.failed && !_.isEmpty(_.trim(item.result.input))) {
              const error = item.error({
                arg: item.result.input,
              });
              return {
                error: _.isFunction(item.error) ? error : item.error,
              };
  ...
  validateArgument(schema, input) {
    ...
    const op1 = _.indexOf(
      _.map(schema, _.toUpper),
      _.toString(_.toUpper(input))
    );
    const op2 = _.indexOf(schema, input);
    const op3 = _.toUpper(schema) === _.toUpper(input);
  ...
  isCommandAllowed(command, slackResponse, slackData) {
    if (command && command.allowedUsers) {
      return (
        _.includes(command.allowedUsers, slackResponse.user) ||
        this.validateGroup(
          slackResponse.user,
  ...
  shouldBlockDirectMessage(command, slackResponse) {
    if (command && command.blockDirectMessage && responseHandler.isDirectMessage(slackResponse)) {
  ...
  validateGroup(currentUser, allowedUsers, usergroups) {
    if (usergroups) {
      const filteredUserGroup = _.flatten(
        _.map(allowedUsers, function (allowedUser) {
...
const getParams = function (slackResponse, level, realValue = false) {
  ...
  if (paramsLen) {
    ...
    if (_.isUndefined(level) || _.isNull(level)) {
      ...
      return _.map(_.get(slackResponse, 'message.params', [], _.toUpper));
    }
