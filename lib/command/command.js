...
const Command = class {
  ...
  validateCommandArgs(command, parsedMessage) {
    ...
    if (!result.isValid) {
      _.merge(result, {
        noOfErrors: _.get(validationResult, [0, 'noOfErrors']),
        cause: _.pick(_.nth(validationResult), ['schema', 'help']),
        failedParams: _.compact(
          _.map(_.nth(validationResult).help, (item, index) => {
  ...
  validateArgument(schema, input) {
    ...
    if (
      _.includes(_.map(schema, _.toUpper), _.toString(_.toUpper(input))) ||
