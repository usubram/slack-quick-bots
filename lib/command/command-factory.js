  ...
  handleMessage (parsedMessage) {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        const command = this.commandObj[_.get(parsedMessage,
          'message.command')];
        if (command) {
    ...
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        ...
        if (requestData && hookInstance && commandModel) {
          const template = _.get(commandModel, 'command.template', _.noop);
          const renderedData = requestData.text || template(requestData);
          onFulfill({
