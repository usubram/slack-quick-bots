      ...
      then: (onFulfill, onReject) => {
        try {
          const job = new CronJob({
            ...
            onTick: function () {
              const scheduleCommand = getCommandArguments(
                parsedMessage);
      ...
      then: (onFulfill, onReject) => {
        ...
        try {
          this.getCommand(_.get(scheduleCommand, 'message.command')).data
            ...
            }, _.merge({}, this.buildOptions(scheduleCommand,
              this.getSlackData(), this.purpose), { isScheduled: true }),
            this.callback,
            ]);
        } catch (err) {
  ...
  */
  validate (parsedMessage) {
    ...
    return Promise.resolve(this.isCommandValid(scheduleCommand, command))
      .then(() => {
        return this.isCronValid(getCronExpresion(parsedMessage),
          parsedMessage);
      });
  }
