...
externals.Schedule = class extends Command {
  ...
  */
  setTimer (parsedMessage, job) {
    ...
    this.getEventStore().set([scheduleCommand.channel +
      '_SCHEDULE_' + scheduleTask, 'timer'], job);
  }
  ...
  */
  setEvent (parsedMessage) {
    return this.getEventStore().update('schedule', {
      parsedMessage: parsedMessage,
      channels: [parsedMessage.channel],
    }).then(() => parsedMessage);
  }
  ...
  */
  getScheduledEvents (parsedMessage) {
    ...
    const allowedEvents = _.filter(_.values(this.getEventStore()
      .getSchedules()), (item) => {
      const eventChannel = _.get(item, 'parsedMessage.channel');
      const sameChannel = eventChannel === channel;
      const isSchedule = _.get(item,
        'parsedMessage.message.command') === 'SCHEDULE';
      const isPublic = isSchedule ?
        responseHandler.isPublicMessage(item.parsedMessage) : false;

      return isSchedule &&
        (sameChannel || isPublic || (sameChannel && !isPublic));
    });
