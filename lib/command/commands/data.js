  ...
  preprocess (parsedMessage) {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        ...
        try {
          this.template = this.getCommand().template ? this.getCommand()
            .template : '';
        } catch (err) {
          logger.error(
      ...
      then: (onFulfill, onReject) => {
        try {
          ...
          if (localDataFunction.length < 3) {
            return localDataFunction.apply(this, dataFunctionArguments)
              .then((data) => onFulfill(this.message.bind(this, parsedMessage)(null, data)))
              .catch((error) => onFulfill(this.message.bind(this, parsedMessage)(error)));
          }

          return localDataFunction.apply(this, dataFunctionArguments.concat(this.callback));
        } catch (err) {
