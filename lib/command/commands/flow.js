...
import postMessage from '../../slack-api/post-message.js';

const { isEmpty, get, compact, isFunction, find, take } = lodash;
/**
 *
...
const Flow = class extends Command {
   ...
   * @override
   */
  preprocess(parsedMessage, validationResult) {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        this.template = '';
        this.purpose = isEmpty(this.getHook())
          ? ''
        ...
        const timer = get(this.getEventStore().get(), this.timerKey);
        ...
        if (
          (question || []).length === 0 ||
          (validationResult.isPresetValid &&
            validationResult.isPresetValid.length > 0)
        ) {
  ...
  message(parsedMessage, err, data) {
    ...
    if (get(data, 'terminated') === true) {
      ...
      if (get(data, 'error')) {
        ...
        } else if (get(data, 'responseType') === 'rich') {
          postMessage(
            {
            ...
            Object.assign(data.response, {
              ...
              thread_ts:
                parsedMessage.thread_ts || get(data, 'thread')
                  ? parsedMessage.thread_ts || parsedMessage.ts
                  : '',
            })
          );
        }
    ...
    if (this.getCommand().responseType || get(data, 'responseType')) {
      if (get(data, 'responseType') === 'rich') {
        postMessage(
          {
            botToken: this.getBotConfig().botToken,
            agent: this.getHttpAgent(),
    ...
    } else if (data && isFunction(this.template)) {
      try {
        renderedData = this.template(data);
  ...
  buildOptions(slackResponse, slackData, purpose) {
    ...
    const validation = get(this.getEventStore().get(), this.questionKey, []);
    let response = {};
    if (validation) {
  ...
  invalidateFlow(questionKey, timerKey) {
    clearTimeout(this.getEventStore().get(timerKey));
  ...
  getFlowValidations(parsedMessage) {
    ...
    const alreadyAnswered = validations.filter((item) => item.answered);
    const pickCount = alreadyAnswered.length > 0 ? 3 : 2;
    return take(
