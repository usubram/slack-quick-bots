  ...
  process (parsedMessage) {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        ...
        _.each(_.values(alertEvents), async (alertEvent) => {
          const shouldSnooze = await this.getCommand().snooze.apply(this, [{
...
const computeCumulativeDifference =
  function (timeSeriesDataSet, previousDataSet, isPercentage) {
    ...
    if (lastPreviousData &&
      ...
      const difference = _.last(sortedDataSetByTime).value -
