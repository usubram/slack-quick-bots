...
const Alert = class extends Command {
    ...
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        if (
          ...
          const isAlertExist = filter(values(alertEvents), (alertEvent) => {
            return (
              alertEvent &&
              isEqual(
                currentParams,
          ...
          if (isAlertExist.length > 0) {
            ...
            return onReject(
              new logger.error('Alert with arguments already exist')
            );
          }

          let time = this.getParams(parsedMessage, 0);
          time = isNumber(time) && time > 0 ? time : 1;
  ...
  process(parsedMessage) {
    return Promise.resolve({
      then: (onFulfill) => {
        ...
        each(values(alertEvents), (alertEvent) => {
          if (typeof this.getCommand().snooze === 'function') {
            ...
            if (shouldSnooze) {
              logger.info('Snoozing alert', { time: new Date() });
              return;
            }
          }
          ...
          alertEvent.handler.apply(this, [
            ...
            partial(callback, alertEvent.parsedMessage.scheduleId),
          ]);
        });
      ...
      then: (onFulfill) => {
        if (
          ...
          this.messageHandler({
            channels: parsedMessage.channel,
            message: generateBotResponseTemplate({
              template: isFunction(this.template) ? this.template : '',
              /* jshint ignore:start */
  ...
  setupAlertTypeSpecifics(parsedMessage) {
    ...
    set(channels, parsedMessage.scheduleId, {
      parsedMessage: parsedMessage,
    ...
    events = merge(
      {},
      get(this.getEventStore().get(), this.alertTaskChannelPath, {}),
      channels
    );
  ...
  handleCumulativeDifference(data, eventKey) {
    if (data) {
      const alertData = get(
        this.getEventStore().get(),
        concat(this.alertTaskChannelPath, eventKey)
      );
      const sensitivity = get(alertData, 'sensitivity');
      const isPercentage = endsWith(sensitivity, '%');
      const cResult = computeCumulativeDifference(
        data,
        get(alertData, 'previousDataSet'),
        isPercentage
      );
      ...
      if (Math.abs(result.difference) > parseInt(sensitivity, 10)) {
        this.messageHandler({
          ...
          message: generateAlertResponseTemplate(result),
        });
      }
    }
  }
  ...
  handleVariationDifference(data, eventKey) {
    ...
    if (data && data.length > 0) {
      ...
      if (varianceResult && varianceResult.perct) {
        alertData = get(this, concat(this.alertTaskChannelPath, eventKey));
      }

      if (varianceResult.perct > get(alertData, 'sensitivity')) {
        this.messageHandler({
          channels: [alertData.parsedMessage.channel],
          message: generateAlertResponseTemplate(
            extend(
              {
                template: this.template,
...
const computeCumulativeDifference = function (
  ...
  const sortedDataSetByTime = sortBy(timeSeriesDataSet, 'time');
  const lastPreviousData = last(previousDataSet) || last(sortedDataSetByTime);
  const currentDataTime = get(last(sortedDataSetByTime), 'time');
  const isFuture = currentDataTime
    ? unix(currentDataTime).isAfter(unix(lastPreviousData.time))
    : false;
  let result = {};

  if (lastPreviousData && isFuture) {
    const lastValue = last(sortedDataSetByTime).value;
    const firstValue = first(sortedDataSetByTime).value;
  ...
  result.previousDataSet =
    size(sortedDataSetByTime) > 0 ? sortedDataSetByTime : previousDataSet;
...
const calculateVariance = function (dataSetArr) {
  const dataSetLength = isArray(dataSetArr) ? dataSetArr.length : 0;
  let sensitivityPercentage = 0;

  if (dataSetLength > 0 && dataSetLength % 2 === 0) {
    const sdSet1 = Math.abs(
      standardDeviation(slice(dataSetArr, 0, dataSetLength / 2))
    );
    const sdSet2 = Math.abs(
      standardDeviation(slice(dataSetArr, dataSetLength / 2))
    );
