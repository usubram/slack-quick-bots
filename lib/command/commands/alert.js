...
externals.Alert = class extends Command {
    ...
    return Promise.resolve({
      then: (onFulfill) => {
        if (this.getParams(parsedMessage, 0) === internals.alertParams[0]) {
          this.alertTaskPath = ['eventStore', parsedMessage.channel +
            '_' + parsedMessage.message.command];
          this.alertTaskChannelPath = _.concat([
            parsedMessage.message.command], 'channel');
          this.alertTaskTimerPath = _.concat([
            parsedMessage.message.command], 'timer');
          this.alertCurrentSentivityPath = _.concat(this.alertTaskChannelPath,
              parsedMessage.channel, 'sentivity');
          this.alertCurrentMessagePath = _.concat(this.alertTaskChannelPath,
              parsedMessage.channel, 'parsedMessage');
  ...
  */
  process () {
    const parsedMessage = _.get(this.eventStore, this.alertCurrentMessagePath);

    return Promise.resolve({
      then: (onFulfill, onReject) => {
        this.callback = (data) => {
          onFulfill(this.message.bind(this)(data));
        };
  ...
  */
  setupAlertTypeSpecifics (parsedMessage) {
    ...
    _.set(this.eventStore, this.alertCurrentSentivityPath, sentivity);
    _.set(this.eventStore, this.alertCurrentMessagePath, parsedMessage);

    if (!_.get(this.eventStore, this.alertTaskTimerPath, undefined)) {
      _.set(this.eventStore, this.alertTaskTimerPath, setInterval(() => {
        this.quietRespond();
      }, time));
    }
  }
  ...
  */
  handleAlertMessage (data) {
    if (this.getCommand().algo === internals.algo.CUMULATIVE_DIFFERENCE) {
      this.handleCumulativeDifference(data);
    } else if (this.getCommand().algo ===
      internals.algo.CONSISTENT_VARIATION) {
      this.handleVariationDifference(data);
    }
  }
  ...
  * @param {object} data result from alert data handler.
  */
  handleCumulativeDifference (data) {
    let channelsToAlert = [];

    if (data) {
      const cResult = internals.computeCumulativeDifference(data);
      const alertData = _.get(this.eventStore, this.alertTaskChannelPath);
      ...
      channelsToAlert = _.flatten(_.compact(_.map(alertData,
        (value, key) => {
          if (result.difference > _.get(this.eventStore,
            _.concat(this.alertTaskChannelPath, key, 'sentivity'), 0)) {
            return key;
          }
        })
      ));
      ...
      if (channelsToAlert && channelsToAlert.length > 0) {
        this.messageHandler({
          ...
          message: responseHandler.generateAlertResponseTemplate(_.extend({
            template: this.template,
            consistent_variation_alert: true,
            dataset: data,
