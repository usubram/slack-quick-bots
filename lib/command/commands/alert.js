  ...
  preprocess (parsedMessage) {
    return Promise.resolve({
      then: (onFulfill, onReject) => {
        if (this.getParams(parsedMessage, 0) ===
          DEFAULT_CONFIG.alertParams[0]) {
  ...
  */
  handleAlertMessage (data, eventKey) {
    if (this.getCommand().algo === DEFAULT_CONFIG.algo.CUMULATIVE_DIFFERENCE) {
      ...
      DEFAULT_CONFIG.algo.CONSISTENT_VARIATION) {
  ...
  */
  handleCumulativeDifference (data, eventKey) {
    if (data) {
      ...
      const cResult = computeCumulativeDifference(data,
...
*/
const computeCumulativeDifference =
  function (timeSeriesDataSet, previousDataSet, isPercentage) {
    ...
    const currentDataTime = _.get(
      _.last(sortedDataSetByTime), 'time');
    const isFuture = currentDataTime ? moment.unix(currentDataTime)
      .isAfter(moment.unix(lastPreviousData.time)) : false;
    let result = {};
    ...
    if (lastPreviousData && isFuture) {
      ...
      if (isPercentage) {
        const differencePerct = Math.abs(Math.round(difference /
          firstValue * 100));
...
const calculateVariance = function (dataSetArr) {
  ...
  if (dataSetLength > 0 && dataSetLength % 2 === 0) {
    const sdSet1 = Math.abs(standardDeviation(
      _.slice(dataSetArr, 0, (dataSetLength / 2))));
    const sdSet2 = Math.abs(standardDeviation(
