...
externals.Alert = class extends Command {
    ...
    return Promise.resolve({
      then: (onFulfill) => {
        if (this.getParams(parsedMessage, 0) === internals.alertParams[0]) {
          this.alertTaskPath = [parsedMessage.message.command];
          this.alertTaskChannelPath = _.concat([
            parsedMessage.message.command], 'channel');
          this.alertTaskTimerPath = [parsedMessage.message.command, 'timer'];
  ...
  */
  setupAlertTypeSpecifics (parsedMessage) {
    ...
    this.getEventStore().set(this.alertCurrentSentivityPath, sentivity);
    this.getEventStore().set(this.alertCurrentMessagePath, parsedMessage);
  }
  ...
  handleCumulativeDifference (data) {
    ...
    if (data) {
      ...
      channelsToAlert = _.flatten(_.compact(_.map(alertData,
        (value, key) => {
          if (result.difference > _.get(this.getEventStore().get(),
