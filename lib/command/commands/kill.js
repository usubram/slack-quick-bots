  ...
  */
  respond (parsedMessage) {
    ...
    const targetSchedule = _.get(this.getEventStore().getSchedules(),
      [scheduleId, 'parsedMessage', 'channel']);
    const scheduledUser = _.get(this.getEventStore()
      .getSchedules(), [scheduleId, 'parsedMessage', 'user']);
      ...
      if (parsedMessage.channel !== targetSchedule) {
        this.messageHandler({
          ...
          message: responseHandler.generateBotResponseTemplate({
            ...
            command: killTask,
            /* jshint ignore:start */
            schedule_previlage: true,
            bot_name: this.getEventStore().botName,
            target_channel: targetSchedule,
            thread: parsedMessage.thread_ts,
            /* jshint ignore:end */
          }),
        });
