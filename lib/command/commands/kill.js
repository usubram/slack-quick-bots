...
externals.Kill = class extends Command {
  ...
  */
  respond (parsedMessage) {
    ...
    const scheduleTaskPath = [parsedMessage.channel + '_SCHEDULE_' +
      scheduleTask, 'timer'];
    ...
    if (recursiveTimer) {
      ...
      this.getEventStore().remove('events', {
        parsedMessage: parsedMessage,
        channels: [parsedMessage.channel],
      ...
      this.getEventStore().remove('schedule', {
        parsedMessage: parsedMessage,
        channels: [parsedMessage.channel],
        commandToKill: [parsedMessage.channel +
          '_SCHEDULE_' + scheduleTask],
      }).catch((err) => {
