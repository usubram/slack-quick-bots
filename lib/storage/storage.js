  ...
  return Promise.resolve(exports.readFile(eventType))
    .then((eventsData) => {
      if (data && data.parsedMessage && data.channels) {
        ...
        _.set(result, botName, internals.pickEvents(eventType,
          _.get(eventsData, botName, {}), data));
        ...
        .then((eventsData) => {
          if (_.get(data, 'channels', []).length) {
            _.forEach(data.channels, function (channel) {
              const eventPath = _.concat(botName, _.get(data, 'commandToKill'));
              const compatibleEventPath = [botName,
...
internals.pickEvents = function (eventType, storeData, newData) {
  const scheduleTask = _.toUpper(
    _.get(newData, 'parsedMessage.message.params[0]'));
  ...
  _.forEach(newData.channels, function (channel) {
    if (eventType === 'events') {
      ...
      _.set(storeData, channel + '_SCHEDULE_' + scheduleTask, newData);
    }
  });
