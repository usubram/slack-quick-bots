...
const EVENT_FILE_PATH = path.join(STORAGE_DIRECTORY, 'events.json');
const SCHEDULE_FILE_PATH = path.join(STORAGE_DIRECTORY, 'schedule.json');
      ...
      .then((eventsData) => {
        if (data && data.parsedMessage && data.channels) {
          ...
          _.set(
            result,
            options.botName,
            pickEvents(options, _.get(eventsData, options.botName, {}), data)
          );
        ...
        this.readFile(options.eventType)
          .then((eventsData) => {
            if (_.get(data, 'channels', []).length) {
              _.forEach(data.channels, function () {
                const eventPath = _.concat(
                  options.botName,
                  _.get(data, 'commandToKill')
                );
...
const pickEvents = function (options, storeData, newData) {
  const eventTask = _.toUpper(_.get(newData, 'parsedMessage.message.command'));
