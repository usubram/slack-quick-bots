...
exports.createEventDirectory = function () {
  fs.mkdir(internals.STORAGE_DIRECTORY, function (e) {
    if (!e || (e && e.code === 'EEXIST')) {
      botLogger.logger.info('Data directory already exist, so not creating');
    } else {
      botLogger.logger.error('Unable to create storage for ' +
...
exports.updateEvents = function (botName, eventType, data) {
  return Promise.resolve(exports.readFile(eventType))
    .then((eventsData) => {
      ...
      botLogger.logger.info('Events updates successfully');
      botLogger.logger.debug('Event: ', responseData);

      return Promise.resolve(responseData);
    }).catch((err) => {
      botLogger.logger.info('Events update failed');
      botLogger.logger.debug('Event: ', err);
    ...
    then: (onFulfill, onReject) => {
      exports.readFile(eventType)
        .then((eventsData) => {
          ...
          botLogger.logger.info('Events removed successfully');
          botLogger.logger.debug('Events removed successfully for ', responseData);
          onFulfill(responseData);
        }).catch((err) => {
          botLogger.logger.info('Events removed failed');
          botLogger.logger.debug('Events remove error with ', err);
          onReject(err);
        });
    }
  });
    ...
    then: (onFulfill, onReject) => {
      fs.readFile(path, { encoding: 'utf8', flag: 'a+' }, function (err, data) {
        ...
        try {
          ...
          botLogger.logger.info('Reading event file failed');
          botLogger.logger.debug('Reading file event failed', parseErr, path);
  ...
  return Promise.resolve({
    then: (onFulfill, onReject) => {
      fs.writeFile(path, fileData, { encoding: 'utf8', flag: 'w+' }, function (err) {
        if (err) {
          botLogger.logger.info('Write event to file failed');
          botLogger.logger.debug('Write event file failed', err, path);
          return onReject(err);
        }
        onFulfill(data);
      });
